...
public class GeoLocationService implements GeoLocation {
    ...
    public static final String USER_IS_NOT_ELIGIBLE_TO_REGISTER = "User is not eligible to register";
    @Autowired
    ...
    public UserDetailsReponseDto findUserDetailsByIp(UserDetailsRequestDto userDetailsRequestDto) throws RuntimeException {
        ...
        if(StringUtils.hasLength(city)) {
            ...
            throw new InvalidIPProvideException(USER_IS_NOT_ELIGIBLE_TO_REGISTER);
        }
    }

    private String findGeoLocationByIp(String ip) {
        GeoApiResponseDto geoApiResponseDto = getGeoApiResponseDto(ip);
        if(StringUtils.hasLength(geoApiResponseDto.getCountryCode()) && validator.isValidCAUser(geoApiResponseDto.getCountryCode()))
